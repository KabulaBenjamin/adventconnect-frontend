...
import { Image as ImageIcon, X, Loader, Sparkles, Search } from 'lucide-react';
    ...
    <div className="w-full max-w-2xl mx-auto space-y-5 pb-10 text-left">
      ...
      <div className="relative group">
        ...
        <input
          ...
          placeholder="Search sanctuary assets..."
      ...
      <form onSubmit={handlePostSubmit} className="bg-white/70 backdrop-blur-md p-5 rounded-3xl border border-white shadow-xs focus-within:bg-white transition-all duration-300">
        <div className="flex items-start gap-3.5">
          ...
          <textarea
            ...
            className="w-full border-none focus:ring-0 text-xs resize-none outline-none bg-transparent pt-2.5 placeholder:text-slate-300 font-bold text-slate-700"
            rows={2}
          />
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {posts.map((post) => <PostItem key={post._id} post={post} user={user} fetchPosts={fetchPosts} />)}
        </div>
      )}
    </div>
  );
