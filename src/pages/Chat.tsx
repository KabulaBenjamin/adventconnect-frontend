...
const Chat = () => {
  ...
  // Real-time listener for incoming messages AND reactions
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchChatLogs = async () => {
      ...
      try {
        const history = await apiFetch(`/messages/${selectedUser._id}`);
        setMessages(history || []);
      } catch (err) {
  ...
  // Generalized message sender loop supporting text and stickers
  const handleSendMessagePayload = async (payloadText: string, messageType: 'text' | 'sticker' = 'text', mediaUrl?: string) => {
    ...
    try {
      ...
      // Update message with true database object ID
      setMessages((prev) => prev.map(m => m.tempId === tempId ? savedMsg : m));

      // 2. Emit via socket connection
  ...
  // Toggle reactions on database entry and broadcast live updates
    ...
    <div className="w-full h-screen bg-[#0a0a0a] text-white flex overflow-hidden font-sans antialiased">
      ...
      <div className="w-80 bg-[#121212] border-r border-zinc-900 overflow-hidden hidden md:flex flex-col shrink-0">
        <div className="p-4 pt-5 pb-2">
          <h2 className="font-bold text-xl text-white tracking-tight">Chats</h2>
        </div>
      ...
      <div className="flex-1 bg-[#121212] flex flex-col overflow-hidden relative">
        {selectedUser ? (
          <>
            ...
            <div className="flex-1 bg-[#121212] p-5 overflow-y-auto space-y-6">
              {messages.map((msg, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex flex-col relative group ${isMe ? 'items-start' : 'items-end'}`}
                    onMouseEnter={() => setHoveredMessageId(msgId)}
                    onMouseLeave={() => setHoveredMessageId(null)}
                  >
                    {/* INLINE ACTION REACTION PICKER TOOLBAR (FLIPPED PLACEMENT BASED ON BOXES) */}
                    {hoveredMessageId === msgId && !msgId.startsWith('temp-') && (
                      <div className={`absolute -top-8 z-20 bg-zinc-900 border border-zinc-800 rounded-full px-2 py-1 shadow-xl flex gap-1.5 animate-fade-in ${isMe ? 'left-2' : 'right-2'}`}>
                    ...
                    {/* FLIPPED CONTENT BOX DESIGN INSTRUCTIONS */}
                    <div className={`max-w-xs sm:max-w-md p-3 rounded-2xl relative ${
                      isMe
                        ? 'bg-blue-600 text-white rounded-tl-none font-medium' // SENT BOX (LEFT + BLUE)
                        : 'bg-zinc-800 text-zinc-100 rounded-tr-none font-normal border border-zinc-700/40' // RECEIVED BOX (RIGHT + GREY)
                    }`}>
            ...
            {/* INPUT ACTIONS MENU SLIDERS */}
            <form onSubmit={sendMessage} className="p-4 bg-[#121212] border-t border-zinc-900 flex flex-col gap-3 relative">
              {showEmoji && (
                <div className="absolute bottom-20 left-4 z-50 shadow-2xl rounded-2xl overflow-hidden border border-zinc-800">
                  <EmojiPicker theme={Theme.DARK} onEmojiClick={(e) => setText(prev => prev + e.emoji)} />
                </div>
              )}

              {/* STICKERS MEDIA SELECTION HORIZONTAL COMPONENT */}
              {showStickers && (
                <div className="absolute bottom-20 left-12 z-50 bg-zinc-900 border border-zinc-800 rounded-2xl p-3 shadow-2xl w-72">
                  <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-2 px-1">Expressive Stickers</div>
                  <div className="grid grid-cols-3 gap-2">
                    {STICKERS.map((stk) => (
                      <button
                        ...
                        className="p-1.5 bg-zinc-800 hover:bg-zinc-700/60 rounded-xl transition flex flex-col items-center justify-center group cursor-pointer"
                      >
                        <img src={stk.url} alt={stk.name} className="w-12 h-12 object-contain group-hover:scale-110 transition duration-150" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex gap-2 items-center w-full">
                <div className="flex items-center gap-0.5 text-zinc-500">
                  <button type="button" onClick={() => { setShowStickers(!showStickers); setShowEmoji(false); }} className={`p-2 rounded-full transition cursor-pointer ${showStickers ? 'text-blue-500 bg-zinc-800' : 'text-zinc-400 hover:text-white'}`}>
                    <ImageIcon size={20} />
                  </button>
                  <button type="button" onClick={() => { setShowEmoji(!showEmoji); setShowStickers(false); }} className={`p-2 rounded-full transition cursor-pointer ${showEmoji ? 'text-blue-500 bg-zinc-800' : 'text-zinc-400 hover:text-white'}`}>
                    <Smile size={20} />
                  </button>
                </div>
                ...
                <input
                  ...
                  className="flex-1 bg-zinc-800/80 text-white border border-transparent rounded-full px-4 py-2.5 text-xs focus:outline-none placeholder-zinc-500"
                  placeholder="Message..."
                />
                ...
                <button
                  ...
                  className={`p-2.5 rounded-full transition-all duration-200 ${
                    text.trim() ? 'bg-blue-600 text-white hover:bg-blue-500 cursor-pointer shadow-md' : 'bg-zinc-800 text-zinc-500 cursor-not-allowed'
                  }`}
                >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-zinc-600 gap-3">
            ...
            <p className="font-bold uppercase tracking-widest text-[10px] text-zinc-500">Select a brother or sister to chat</p>
          </div>
        )}
      </div>
      ...
      {selectedUser && showDetailsPanel && (
        <div className="w-80 bg-[#121212] border-l border-zinc-900 flex flex-col overflow-y-auto shrink-0">
          <div className="flex items-center justify-between p-4 border-b border-zinc-900">
            <span className="text-xs font-bold uppercase tracking-wider text-zinc-500">Details</span>
            <button onClick={() => setShowDetailsPanel(false)} className="p-1 hover:bg-zinc-800 rounded-full cursor-pointer"><X size={16} /></button>
          </div>
          <div className="p-4 flex flex-col items-center text-center space-y-4">
            <div className="w-20 h-20 rounded-full bg-zinc-800 flex items-center justify-center font-bold text-2xl">
              {selectedUser.username.charAt(0).toUpperCase()}
            </div>
            <h4 className="text-sm font-bold text-white tracking-wide">{selectedUser.username}</h4>
          </div>
        </div>
      )}
